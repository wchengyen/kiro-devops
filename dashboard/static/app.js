...
const ResourceTreePage = {
  template: `
    <div class="page resource-tree-page">
      ...
      <div class="toolbar">
        ...
        <div class="tag-filter-setting">
          <label>Tag 過濾:</label>
          <input v-model="filterTagKey" @keyup.enter="applyFilter" placeholder="Key" style="width:70px" />
          <input v-model="filterTagValue" @keyup.enter="applyFilter" placeholder="Value (可選)" style="width:100px" />
          <button @click="applyFilter">過濾</button>
          <button v-if="filterTagKey" @click="clearFilter" style="background:var(--text-tertiary)">清除</button>
        </div>
  ...
  setup() {
    ...
    const loadGraph = async () => {
      try {
        let url = "/resource-tree/graph?provider=aws";
        if (filterTagKey.value) {
          url += "&tag_key=" + encodeURIComponent(filterTagKey.value);
          if (filterTagValue.value) {
            url += "&tag_value=" + encodeURIComponent(filterTagValue.value);
          }
        }
